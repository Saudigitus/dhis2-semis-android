...
fun ListingBottomSheet(
    ...
    onConfirm: () -> Unit,
    onError: (@Composable (BottomSheetState.ErrorState) -> Unit)? = null,
) {
    ModalBottomSheet(
        ...
        Column(
            ...
            if (state.isLoading) {
                ...
                when (state) {
                    is BottomSheetState.ErrorState -> {
                        onError?.invoke(state)
                    }
                    is BottomSheetState.HasItemsState -> {
                        LazyColumn(
...
fun ListingBottomSheet(
    ...
    onDismissRequest: () -> Unit,
    onConfirm: () -> Unit,
    handleErrorView: (@Composable (BottomSheetState.ErrorState) -> Unit)? = null,
    handleDataView: @Composable (BottomSheetState) -> Unit,
) {
    ModalBottomSheet(
        onDismissRequest = onDismissRequest,
        sheetState = rememberModalBottomSheetState(
        ...
        Column(
            ...
            if (state.isLoading) {
                ...
                when (state) {
                    is BottomSheetState.ErrorState -> {
                        handleErrorView?.invoke(state)
                    }
                    is BottomSheetState.HasItemsState -> {
                        handleDataView.invoke(state)
                    }
                    is BottomSheetState.GenericsState<*> -> {
                        handleDataView.invoke(state)
                    }
                }
            }
